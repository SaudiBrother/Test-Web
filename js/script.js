...
function showScreen(screenName) {
    try {
        ...
        if (backContainer) {
            backContainer.classList.remove('expanded');
        }
    } catch (error) {
...
function showFunFacts() {
    try {
        ...
        funFactsData.forEach((fact, index) => {
            ...
            factsContainer.appendChild(factCategory);
        });
...
function controlBackButton(show, targetScreen = null) {
    try {
        ...
        if (!backContainer || !fixedBackButton) return;
        
        if (show) {
            backContainer.style.display = 'flex';
            fixedBackButton.onclick = () => {
            ...
            fixedBackButton.onclick = null;
        }
    } catch (error) {
        console.error('Error controlling back button:', error);
    }
}
...
function setupEventListeners() {
    ...
    const themeToggleFacts = document.getElementById('themeToggleFacts');
    ...
    if (themeToggleFacts) {
        themeToggleFacts.addEventListener('click', toggleTheme);
    }
    ...
    if (backArrow) {
        backArrow.addEventListener('click', function() {
            const backContainer = document.querySelector('.back-container');
            if (backContainer) {
                backContainer.classList.toggle('expanded');
            }
        });
    }
    
    // Tutup expanded state tombol kembali saat klik di luar
    document.addEventListener('click', (e) => {
        const backContainer = document.querySelector('.back-container');
        if (backContainer && 
            !backContainer.contains(e.target) && 
            backContainer.classList.contains('expanded')) {
            backContainer.classList.remove('expanded');
        }
    });
